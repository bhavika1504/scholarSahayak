...
from models.admin_analytics_model import AnalyticsModel
...
def overview(current_admin):
    return jsonify({
        "status": True,
        "total_students": AnalyticsModel.total_students(),
        "total_scholarships": AnalyticsModel.total_scholarships(),
        "total_applications": AnalyticsModel.total_applications()
    })
...
def most_applied(current_admin):
    data = AnalyticsModel.most_applied_scholarships()
...
def by_category(current_admin):
    data = AnalyticsModel.applications_by_category()
    return jsonify({"status": True, "data": data})
...
def by_state(current_admin):
    data = AnalyticsModel.applications_by_state()
    return jsonify({"status": True, "data": data})
